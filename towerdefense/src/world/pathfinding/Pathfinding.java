...
public class Pathfinding {
	...
	public static ArrayList<Node> findPath(World world, int xstart,int ystart,int xfinal, int yfinal) {
		
		Vector2i start = new Vector2i(xstart,ystart);
		Vector2i end   = new Vector2i(xfinal,yfinal);
		...
		while(openList.size() > 0 ) {
			...
			for ( int i = 0; i < 9 ; i++) {
				...
				Tile tile = World.tiles[x+x1+((y + y1)*World.getWIDTH())];
				
				if (tile == null) continue;
				if (tile instanceof WallTile) continue;
				if (i == 0) {
					
					Tile aux = World.tiles[x + x1 + 1+((y+y1)*World.getWIDTH())];
					Tile aux2 = World.tiles[x + x1+((y+y1+1)*World.getWIDTH())];
					...
					Tile aux = World.tiles[x + x1+((y+y1-1)*World.getWIDTH())];
					Tile aux2 = World.tiles[x + x1 +1+((y+y1)*World.getWIDTH())];
					if( aux instanceof WallTile || aux2 instanceof WallTile) {
