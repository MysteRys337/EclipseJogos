...
public class World {
	...
	// Variaveis a serem usadas pelo inimigo
	public static int enemySpawnX = 0;
	public static int enemySpawnY = 0;
		...
		for (int x = 0; x < WIDTH; x++) {
			for (int y = 0; y < HEIGHT; y++) {
				...
				switch (this.pixels[pixelAtual]) {
					...
					case 0xFFff0000: // Cor Vermelha| Lugar onde vai ser o spawner das dinamites
						enemySpawnX = x*TILE_SIZE;
						enemySpawnY = y*TILE_SIZE;
						Normal.spawnEnemySpawner(enemySpawnX,enemySpawnY,TILE_SIZE);
						break;
				
					case 0xFF0002ff: 
						spawnX = x*TILE_SIZE;
						spawnY = y*TILE_SIZE;
						tiles[pixelAtual] = new FloorTile(spawnX,spawnY,tileSprites[1].getBufferedImage());
						break;
						
					case 0xFF73a4ff: // Cor Azul | Spawnando tile onde pode ser construido torres
						System.out.println("pixel: " + pixelAtual);
						tiles[pixelAtual] = new ConstructionTile(x*TILE_SIZE,y*TILE_SIZE,tileSprites[2].getBufferedImage(),tileSprites[3].getBufferedImage());
						break;
	...
	public static boolean isConstructableTile(int x,int y) {
		int pixel = (x/TILE_SIZE + (y/TILE_SIZE * WIDTH));
		boolean resp = false;
		
		System.out.println("pixel: " + pixel);
		
		if(tiles[pixel] instanceof ConstructionTile && tiles[pixel].isFree) {
			resp = true;
			tiles[pixel].isFree = false;
		}
