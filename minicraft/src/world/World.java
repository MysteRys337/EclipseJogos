...
public class World {
			...
			for(int y = 0; y < HEIGHT; y++) {
				if(y == (HEIGHT) - 1 || x == (WIDTH) - 1 || x == 0 || y == 0) {
					...
					if ( y > initialStoneHeight) {
						...
						blocks[x+y*WIDTH] = new BackgroundBlock(x*BLOCK_SIZE,y*BLOCK_SIZE,skySprites);
					}
				}
			}
		}
	...
	public static void breakBlock(int x,int y) {
		Sprite[] skySprites = {tileSprites[3],tileSprites[4],tileSprites[5]};
		setBlock(x/BLOCK_SIZE, y/BLOCK_SIZE, new BackgroundBlock(x-(x%BLOCK_SIZE),y-(y%BLOCK_SIZE),skySprites));
	}
